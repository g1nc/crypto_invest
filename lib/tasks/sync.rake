...
namespace :sync do
  task icos: :environment do
    ...
    ws = session.spreadsheet_by_key("19_BCnracLc3rwIhyxmJtfG-fqJqxeFoycXYm3FkkNbI").worksheets[1]
  ...
  task users: :environment do
    ...
    session = GoogleDrive::Session.from_config("config.json")
    ws = session.spreadsheet_by_key("19_BCnracLc3rwIhyxmJtfG-fqJqxeFoycXYm3FkkNbI").worksheets[0]
